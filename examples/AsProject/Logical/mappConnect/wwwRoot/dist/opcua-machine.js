...
export class OpcuaMachine {
    ...
    readGroups = new Map();
    subscriptionUpdateTimers = new Map();
    subscriptionHandles;
    constructor(config) {
     ...
     */
    initCyclicRead(varName, callback, options = {}) {
     ...
     */
    configureReadGroup(readGroupName, options) {
     ...
     * No registration required - automatically resolves NodeId
     */
    async writeVariable(varName, value) {
        // VariableManager already has current defaults - no need to set them again
     ...
     */
    async subscribe(varName, callback, samplingInterval = 100) {
     ...
     */
    async unsubscribe(subscriptionHandle) {
        if (!this.subscriptionHandles) {
     ...
     */
    getVariablesInScope(appModule = '_default', scope = 'AsGlobalPV') {
        const globalState = this.variableManager.getGlobalState();
     ...
     */
    createVariableProxy(appModule, scope, scopeData) {
        const target = this;
    ...
    async createOrUpdateSubscription(readGroupName) {
    ...
    async doCreateOrUpdateSubscription(readGroupName) {
        const group = this.readGroups.get(readGroupName);
    ...
    async deleteSubscription(readGroupName) {
        const group = this.readGroups.get(readGroupName);
...
//# sourceMappingURL=opcua-machine.js.map