...
export class OpcuaConnection {
     ...
     */
    async changeUser(username, password) {
        if (!this.sessionInfo) {
            return rejectWithError(LuxConnectErrorCode.NOT_CONNECTED, 'Not connected to OPC UA server. Call connect() first.');
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                return rejectWithError(LuxConnectErrorCode.AUTHENTICATION_FAILED, `Failed to change user: ${response.status} ${response.statusText} - ${errorText}`, { status: response.status, statusText: response.statusText });
            }
            const responseData = await response.json();
            // Check if the change was successful
            if (responseData.statusCode && responseData.statusCode !== 0) {
                return rejectWithError(LuxConnectErrorCode.AUTHENTICATION_FAILED, `User change failed: ${responseData.statusCode} - ${responseData.description || 'Unknown error'}`, responseData);
            }
            ...
            return rejectWithError(LuxConnectErrorCode.AUTHENTICATION_FAILED, `Failed to change user: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
    /**
    ...
    async apiRequest(endpoint, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                return rejectWithError(LuxConnectErrorCode.SERVER_ERROR, `API request failed: ${response.status} ${response.statusText}`, { status: response.status, statusText: response.statusText, endpoint });
            }
            return response;
        }
        catch (error) {
            return rejectWithError(LuxConnectErrorCode.NETWORK_ERROR, `Network error during API request to ${endpoint}`, { endpoint }, error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
    ...
    extractCookies(response) {
        if (isBrowser)
            return; // Browsers handle cookies automatically
...
Original error: ${message}`);
            ...
            throw this.categorizeConnectionError(error, testUrl);
        }
    }
    /**
     * Categorize connection errors for better diagnostics
     */
    categorizeConnectionError(error, url) {
...
Auth data: ${JSON.stringify(authData)}`);
            ...
            throw this.categorizeConnectionError(error, authUrl);
        }
    }
    /**
    ...
    async recoverConnection() {
        ...
        try {
            ...
            await this.createSession();
            if (this.sessionInfo) {
                console.log('New session created:', this.sessionInfo.sessionId);
            }
            else {
