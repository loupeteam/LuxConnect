import { VariableHierarchy, VariablePathParser } from './variable-hierarchy.js';
...
export class VariableManager {
    ...
    defaultNamespace = 'ns=5;s=';
    defaultApplication = ''; // Empty means use parser default
    defaultTask = 'AsGlobalPV'; // Default task for variables without explicit task
     ...
     */
    createSmartPromise(operation, variableName, fallbackValue) {
     ...
     */
    registerVariable(name, nodeId, readArrayParams = false) {
        if (readArrayParams) {
     ...
     */
    registerVariableSync(name, nodeId) {
        ...
        this.hierarchy.addVariable(normalizedName, nodeId, variableInfo.value, variableInfo.timestamp, variableInfo.quality, variableInfo.dataType);
     ...
     */
    async registerVariableAsync(name, nodeId, readArrayParams) {
        // Validate variable name format
        try {
            VariablePathParser.parse(name);
        }
        ...
        if (readArrayParams) {
            try {
                ...
                if (arrayParams.arrayDimensions) {
                    variableInfo.arrayDimensions = arrayParams.arrayDimensions.map(size => [0, size - 1]);
                }
            }
            catch (error) {
        ...
        // Add to hierarchy using normalized name
        this.hierarchy.addVariable(normalizedName, nodeId, variableInfo.value, variableInfo.timestamp, variableInfo.quality, variableInfo.dataType, variableInfo.valueRank, variableInfo.arrayDimensions);
        // Convert to OpcuaVariable format
        const namedVariable = {
            name: normalizedName,
            nodeId,
     ...
     */
    async readArrayParameters(name) {
        //TODO: This hasn't been tested yet
        const normalizedName = this.normalizeVariableName(name);
        // Get or build nodeId
        let targetNodeId;
     ...
     */
    readValue(name) {
        return this.createSmartPromise(() => this.performReadValue(name), name);
    }
    /**
     * Internal method that performs the actual read operation
     */
    async performReadValue(name) {
        const normalizedName = this.normalizeVariableName(name);
        ...
        if (varData) {
            ...
            try {
                ...
                return rejectWithError(LuxConnectErrorCode.INVALID_VARIABLE_NAME, `Invalid variable name '${name}': ${error}`, { variableName: name }, error instanceof Error ? error : new Error(String(error)));
            }
        }
        ...
        const result = await response.json();
        if (result.status?.code !== 0) {
            return rejectWithError(LuxConnectErrorCode.READ_FAILED, `Failed to read variable '${name}': ${result.status?.code?.description || 'Unknown error'}`, {
                variableName: name,
                nodeId: targetNodeId,
    ...
    writeValue(name, value) {
        return this.createSmartPromise(() => this.performWriteValue(name, value), name);
    }
    /**
     * Internal method that performs the actual write operation
     */
    async performWriteValue(name, value) {
        const normalizedName = this.normalizeVariableName(name);
     ...
     */
    async writeArrayElement(baseVariableName, index, value) {
     ...
     */
    async writeComplexValue(value, originalName) {
     ...
     */
    async executeBatchWrite(writes, originalName) {
        ...
        try {
            ...
            for (const write of writes) {
                const fullVariableName = Object.keys(this.hierarchy.getAllVariables()).find(name => this.hierarchy.getVariable(name)?.mapping.nodeId === write.nodeId);
                if (fullVariableName) {
     ...
     * No registration required - builds NodeId dynamically
     */
    async writeSingleValue(normalizedName, value, originalName) {
        // Try to get nodeId from registered variable first
        let targetNodeId;
        const varData = this.hierarchy.getVariable(normalizedName);
        if (varData) {
     ...
     */
    async writeSingleValueByNodeId(nodeId, value) {
     ...
     */
    removeChangeHandler(name, handler) {
     ...
     */
    removeGlobalChangeHandler(handler) {
     ...
     */
    updateVariableFromNotification(nodeId, value, timestamp, quality) {
    ...
    getRelatedVariables(name) {
        ...
        return related.map(rel => {
            ...
            if (!varData)
                throw new Error(`Related variable ${rel.variable} not found`);
            return {
                type: rel.type,
                variable: {
                    name: varData.mapping.name,
     ...
     */
    emitChangeEvent(name, value, timestamp, quality) {
