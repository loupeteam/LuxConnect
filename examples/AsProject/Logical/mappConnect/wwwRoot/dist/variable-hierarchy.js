...
export class VariablePathParser {
     ...
     */
    static setGlobalDefaultNamespace(namespace) {
     ...
     */
    static parseModuleTaskFormat(variableName) {
     ...
     */
    static parseTaskLocalFormat(variableName) {
        const parts = variableName.split(':');
        const task = parts[0];
     ...
     */
    static parseGlobalFormat(variableName) {
     ...
     */
    static parseVariableWithPath(variableWithPath) {
        const path = [];
     ...
     */
    static buildNodeId(varName, options = {}) {
...
export class VariableHierarchy {
    ...
    arrayDimensionsCache = new Map(); // Cache for array dimensions by base variable name
    /**
     * Add a variable to the hierarchy
     */
    addVariable(name, nodeId, value, timestamp, quality, dataType, valueRank, arrayDimensions) {
     ...
     */
    updateVariable(name, value, timestamp, quality) {
     ...
     */
    findAffectedVariables(changedMapping) {
        const affected = [changedMapping.name]; // Always include the changed variable itself
        for (const [name, mapping] of this.variables) {
            if (name === changedMapping.name)
                continue;
     ...
     */
    isPathAffected(variablePath, changedPath) {
     ...
     */
    getRelationshipType(path1, path2) {
     ...
     */
    ensurePathSegment(current, segment, isArray) {
        if (this.isArrayIndex(segment)) {
     ...
     */
    setPathSegmentValue(current, segment, value) {
        if (this.isArrayIndex(segment)) {
     ...
     */
    shouldMergeObjects(existing, newValue) {
     ...
     */
    mergeArraysByIndex(existingArray, newArray) {
     ...
     */
    mergeObjects(existingObj, newObj) {
    ...
    */
     ...
     */
    extractBaseArrayName(variableName) {
        const firstBracket = variableName.indexOf('[');
        if (firstBracket === -1)
            return null; // No array syntax
...
//# sourceMappingURL=variable-hierarchy.js.map